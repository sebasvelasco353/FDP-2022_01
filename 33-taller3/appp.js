...
function drawElements() {
    ...
    const valueCIUDAD = document.getElementById("FiltroCiudad").value;
    const results = DAYS.filter(day => day.city === valueCIUDAD);

    for (let i = 1; i > 0; i--) {
        const element = results[i];
        element.dCiudades(document.getElementById('City'));
    }

    const valueDIA = document.getElementById("FiltroDia").value;
    const D = results.filter(day => day.day === valueDIA);
