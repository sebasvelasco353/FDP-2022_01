...
let Grupo_sanguineo = document.getElementById('Grupo_sanguineo')
...
let grupoSanguineo = ['A+', 'B+', 'O+', 'AB+', 'A-', 'B-', 'O-', 'AB-'];

function mostrarPaises(arreglo, Lugar) {
...
mostrarPaises(paises, Nacionalidad)

function mostrarGrupos(arreglo, grupo) {
    let elementos2 = '<option selected disables>--Seleccione--</option>'

    for(let i = 0; i < arreglo.length; i++) {
        elementos2 += '<option value="' + arreglo[i] +'">' + arreglo[i] + '</option>'
    }

    grupo.innerHTML = elementos2

}

mostrarGrupos(grupoSanguineo, Grupo_sanguineo)
