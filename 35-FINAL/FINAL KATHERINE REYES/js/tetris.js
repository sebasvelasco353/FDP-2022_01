class Game {
    ...
    // Tiempo de animación cuando se borra una fila
    static DELETE_ROW_ANIMATION = 500;
    // Puntuación a añadir cuando una casilla desaparece (para cada casilla)
    ...
    showWelcome() {
        Swal.fire("Bienvenido", `TETRIS GAME ONLINE
<br>
...
<li class="list-group-item"><strong>También puedes usar los botones si estás en celular</strong></li>
</ul>
</br>
`);
    }
    ...
    mainLoop() {
        if (!this.canPlay) {
           ...
           // Si la figura puede bajar, baja
        if (this.figureCanMoveDown()) {
            this.globalY++;
        } else {
            // Si la figura no puede, entonces iniciamos un tiempo de espera porque
            // el jugador puede mover la figura para que siga bajando
            ...
            setTimeout(() => {
                this.timeoutFlag = false;
            // Si el tiempo expira, volvemos a comprobar si la figura no puede seguir bajando. Si puede
            // (porque el jugador la ha movido) entonces volvemos y mantenemos el bucle
               
            if (this.figureCanMoveDown()) {
                    return;
                }
            // En este punto, sabemos que la figura colapsó con el suelo
            // o con otro punto. Así que movemos toda la figura al array de piezas existente
    ...
    initSounds() {
        this.sounds.background = Utils.loadSound("sonidos/New Donk City_ Daytime 8 Bit.mp3", true);
        this.sounds.success = Utils.loadSound("sonidos/success.wav");
        this.sounds.denied = Utils.loadSound("sonidos/denied.wav");
        this.sounds.tap = Utils.loadSound("sonidos/tap.wav");
    }
    ...
    // Devuelve true aunque el punto no sea válido (por ejemplo si está fuera de límite, porque no es responsabilidad de la función)
